...
// commsRelayDone and commsConvoChoice1 now live in gameState
...
function commsStartRelaySequence() {
    ...
    commsShowChoice(['Yes', 'No'], (sel) => {
        if (sel === 1) {
            ...
            gameState.commsRelayDone = true; // prevent re-triggering
...
function onConvoChoice1(sel) {
    gameState.commsConvoChoice1 = sel;
...
function onConvoChoice2(sel) {
    ...
    const playerText = gameState.commsConvoChoice1 === 0
        ? 'No idea...'
    ...
    let friendResponse;
    if (gameState.commsConvoChoice1 === 0) {
...
function onConvoChoice3(sel) {
    ...
    commsQueueActions([
        ...
        { type: 'callback', fn: () => { gameState.commsComplete = true; } },
    ]);
}
