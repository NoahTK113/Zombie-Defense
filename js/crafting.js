...
// --- Crafting Noise Texture ---
const craftNoiseCanvas = document.createElement('canvas');
craftNoiseCanvas.width = 256;
craftNoiseCanvas.height = 256;
{
    const nctx = craftNoiseCanvas.getContext('2d');
...
function drawCrafting() {
    ...
    for (let nx = 0; nx < canvas.width; nx += 256) {
        for (let ny = 0; ny < canvas.height; ny += 256) {
            ctx.drawImage(craftNoiseCanvas, nx, ny);
        }
    }
    ctx.globalAlpha = 1.0;
