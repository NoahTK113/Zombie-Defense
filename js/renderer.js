...
noiseCanvas.height = 256;
{
    ...
    for (let i = 0; i < nd.length; i += 4) {
        ...
        nd[i]     = Math.floor(v * 0.3);
        nd[i + 1] = Math.floor(v * 0.5);
        nd[i + 2] = v;
        nd[i + 3] = 255;
    }
...
function draw() {
    ...
    const artifactDmgRatio = 1 - artifactHP / ARTIFACT_MAX_HP;
    ...
    // Draw stars behind terrain and all entities
    {
        ...
        for (const star of stars) {
            ...
            const br = artifactCorrupted ? Math.max(star.brightness, 0.7) : star.brightness;
    ...
    // Draw visible chunks
    const chunkScreenSize = CHUNK_PX * effectiveZoom;
    const startCX = Math.floor(camX / CHUNK_PX);
    const startCY = Math.floor(camY / CHUNK_PX);
    const endCX = Math.ceil((camX + viewW) / CHUNK_PX);
    const endCY = Math.ceil((camY + viewH) / CHUNK_PX);
    ...
    for (let cy = Math.max(0, startCY); cy <= Math.min(CHUNKS_Y - 1, endCY); cy++) {
        for (let cx = Math.max(0, startCX); cx <= Math.min(CHUNKS_X - 1, endCX); cx++) {
            ...
            if (!chunk) continue;
            const sx = Math.round((cx * CHUNK_PX - camX) * effectiveZoom);
            const sy = Math.round((cy * CHUNK_PX - camY) * effectiveZoom);
    ...
    // Draw artifact animated overlay (pulsing fractal veins, tinted by HP)
    {
        ...
        if (sx + sw > 0 && sx < canvas.width && sy + sh > 0 && sy < canvas.height) {
            if (artifactCorrupted) {
                veinTintCanvas.width = artifactVeinCanvas.width;
                veinTintCanvas.height = artifactVeinCanvas.height;
                veinTintCtx.clearRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                for (let ox = -1; ox <= 1; ox++) {
                    for (let oy = -1; oy <= 1; oy++) {
                        veinTintCtx.drawImage(artifactVeinCanvas, ox, oy);
                    }
                }
                ...
                veinTintCanvas.height = artifactVeinCanvas.height;
                veinTintCtx.clearRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                veinTintCtx.drawImage(artifactVeinCanvas, 0, 0);
                veinTintCtx.globalCompositeOperation = 'source-in';
                veinTintCtx.fillStyle = `rgb(${veinR}, ${veinG}, ${veinB})`;
                veinTintCtx.fillRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                veinTintCtx.globalCompositeOperation = 'source-over';
    ...
    // Draw channel energy vein (same tint as artifact veins)
    {
        ...
        if (sx + sw > 0 && sx < canvas.width && sy + sh > 0 && sy < canvas.height) {
            if (artifactCorrupted) {
                veinTintCanvas.width = channelVeinCanvas.width;
                veinTintCanvas.height = channelVeinCanvas.height;
                veinTintCtx.clearRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                for (let ox = -1; ox <= 1; ox++) {
                    for (let oy = -1; oy <= 1; oy++) {
                        veinTintCtx.drawImage(channelVeinCanvas, ox, oy);
                    }
                }
                veinTintCtx.globalCompositeOperation = 'source-in';
                veinTintCtx.fillStyle = `rgb(${veinR}, ${veinG}, ${veinB})`;
                ...
                veinTintCanvas.height = channelVeinCanvas.height;
                veinTintCtx.clearRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                veinTintCtx.drawImage(channelVeinCanvas, 0, 0);
                veinTintCtx.globalCompositeOperation = 'source-in';
                veinTintCtx.fillStyle = `rgb(${veinR}, ${veinG}, ${veinB})`;
                veinTintCtx.fillRect(0, 0, veinTintCanvas.width, veinTintCanvas.height);
                veinTintCtx.globalCompositeOperation = 'source-over';
    ...
    if (gameMode === 'build') {
        ...
        if (tile.x >= 0 && tile.x < WORLD_W && tile.y >= 0 && tile.y < WORLD_H) {
            ...
            if (t === TILE.AIR && !playerOccupiesTile(tile.x, tile.y) && points >= 5) {
    ...
    if (shovelTargetBlock && gameMode === 'player' && !player.dead) {
    ...
    if (isIntroLying || isIntroStanding) {
        ...
        if (gameMode === 'player' && shovelSwingTimer > 0) {
    ...
    if (!intro.active && commsComplete && gameMode === 'player' && !player.dead && !artifactCorrupted && !artifactUI.open && artifactUI.flashPhase === 'none' && isNearArtifact()) {
        ctx.save();
        ctx.font = 'bold 32px Jura, sans-serif';
        ctx.textAlign = 'center';
    ...
    if (!intro.active && gameMode === 'build') {
        ctx.save();
        ctx.font = 'bold 32px Jura, sans-serif';
        ctx.textAlign = 'center';
        const pulse = 0.6 + 0.4 * Math.sin(gameTime * 3);
    ...
    // Tutorial prompts
    if (!intro.active && tutorialStep === 'flashlight') {
        ctx.save();
        ctx.font = 'bold 32px Jura, sans-serif';
        ctx.textAlign = 'center';
        const pulse = 0.5 + 0.5 * Math.sin(gameTime * 3);
        ...
        ctx.restore();
    } else if (!intro.active && tutorialStep === 'comms') {
        ctx.save();
        ctx.font = 'bold 32px Jura, sans-serif';
        ctx.textAlign = 'center';
        const pulse = 0.5 + 0.5 * Math.sin(gameTime * 3);
    ...
    if (hasUsedArtifact && !intro.active) {
        ctx.save();
        const hudX = 20;
        ...
        ctx.fillText(`${points} KJ`, hudX, hudY);
    ...
    if (!intro.active && gameMode === 'player' && !player.dead) {
        ctx.save();
        const hudX = 20;
    ...
    if (debugShowFlowField && flowField) {
        ctx.save();
    ...
    ctx.fillText(`[F1] Skip Intro  [F2] Skip Comms  [F3] Skip Artifact  [F4] Start Wave  [F5] Wave Skip  [F6] Respawn  [F7] Restore  [F8] Target: ${debugNoTargetPlayer ? 'OFF' : 'ON'}  [F9] Clear Weapons  [F10] +1000 KJ  [F11] FlowField: ${debugShowFlowField ? 'ON' : 'OFF'}  [Ctrl+S] Save  [Ctrl+L] Load`, dbx, canvas.height - 10);
    ctx.restore();
    ...
    if (artifactUI.flashPhase === 'circles_fadein' || artifactUI.open) {
        ...
        const t = gameTime;
        const conduitAvailable = wave.state === 'idle' && !artifactCorrupted;
    ...
    if (artifactUI.flashPhase === 'boot_text' || artifactUI.flashPhase === 'circles_fadein' || artifactUI.open) {
        ...
        if (artifactUI.bootFirstSession) {
            ...
            ctx.fillText(`E = ${Math.floor(points)} KJ`, bootX, bootY + bootLineH * 2);
    ...
    if (gameOver) {
        ...
        ctx.fillText(`Wave: ${wave.number} | Zombies Killed: ${zombiesKilled} | Survived: ${Math.floor(gameTime)}s`, canvas.width / 2, canvas.height / 2 + 55);
        ctx.restore();
    }
}
